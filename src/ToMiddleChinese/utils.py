...
def format_chinese_text(s, conv):
  def inner(m):
    t = []
    d = []
    for k, v in conv(m[0]):
    ...
    for i, c in enumerate(t):
      if len(c) > 1:
        if l and l[-1] not in punct_dict:
          l += '　'
        l += c
      elif not c or d[i] in minus_signs and d[i + 1] in digits and i and t[i - 1] not in unknown_or_hyphen:
        if not l.endswith('［…］'):
          l += '［…］'
      elif d[i] in decimal_seps and d[i + 1] in digits and i and d[i - 1] in digits:
        continue
      else:
        l += punct_to_fullwidth[c]
    return l
    ...
    for i, (p, c, n) in enumerate(zip(t, t[1:], t[2:]), 1):
      def between():
        ...
        while j and t[j] in right_bracket:
          j -= 1
        f = j and t[j] and (len(t[j]) > 1 or t[j] == 'u')
        j = i + 1
        while j < len(t) - 1 and t[j] in left_bracket:
          j += 1
        g = j and t[j] and (len(t[j]) > 1 or t[j] == 'u')
        return f and g
